...
IF /i %PROCESSOR_ARCHITECTURE% EQU X86 copy %CATALINA_HOME%\bin\Tomcat7.x86.exe %CATALINA_HOME%\bin\tomcat7.exe
IF /i %PROCESSOR_ARCHITECTURE% EQU AMD64 copy  %CATALINA_HOME%\bin\TomEE.amd64.exe %CATALINA_HOME%\bin\tomcat7.exe
IF /i %PROCESSOR_ARCHITECTURE% EQU IA64 copy proc=%CATALINA_HOME%\bin\TomEE.ia64.exe %CATALINA_HOME%\bin\tomcat7.exe
...
:JAVA
"%EXECUTABLE%" //US//%SERVICE_NAME% ++JvmOptions "-Djava.io.tmpdir=%CATALINA_BASE%\temp;-Djava.util.logging.manager=org.apache.juli.ClassLoaderLogManager;-Djava.util.logging.config.file=%CATALINA_BASE%\conf\logging.properties;-Djava.awt.headless=true;-XX:+UseParallelGC;-XX:MaxPermSize=512m" --JvmMs 512 --JvmMx 1024 --JvmSs 2048
goto FINISH
:JAVA7
"%EXECUTABLE%" //US//%SERVICE_NAME% ++JvmOptions "-Djava.io.tmpdir=%CATALINA_BASE%\temp;-Djava.util.logging.manager=org.apache.juli.ClassLoaderLogManager;-Djava.util.logging.config.file=%CATALINA_BASE%\conf\logging.properties;-Djava.net.preferIPv4Stack=true;-Djava.awt.headless=true;-XX:+UseParallelGC;-XX:MaxPermSize=512m" --JvmMs 512 --JvmMx 1024 --JvmSs 2048
:FINISH
